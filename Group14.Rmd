...
We further make a new column which adds the total amount of work per week to the data frames. This is done by multiplying the amount of ECTS by the hours one ects take (25) and by the average weeks of a semester (16). This is our output for the whole tasks that are achieved by a person. This enables us to perform further analysis in this regard.
...
In the summary we cannot immediatly see the a positive relationship between the productivity and the hours of work. 
...
model_transformed_CompositeProductivity<- lm(CompositeProductivity ~ NumberOfPhysicalActivityPerWeek, data = survey_data_transformed)
summary(model_transformed_HoursOfWork)

model_no_outliers_CompositeProductivity <- lm(CompositeProductivity ~ NumberOfPhysicalActivityPerWeek,, data = survey_data_no_outliers)
summary(model_no_outliers_HoursOfWork)


```

The numbers indicate that there are positive relationsships, but none of them 
are statistically significant. This can further be seen in scatter Plots with the linear model. 
