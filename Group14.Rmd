...
Next, we need to separate the combined WorkStudy column into two distinct columns: ECTS and HoursOfWork. This was done because the quastion asked for a answer that combines the two.
This is done using the separate function, with ; as the delimiter. After splitting, we encounter comma-separated values in the ECTS column, which we convert to period-separated values using the str_replace function. This step ensures that the ECTS values are in the correct numeric format and any NA values are replaced with 0 to avoid errors in further calculations.

We proceed to clean and convert the data in other columns. The HoursOfWork column is converted to a numeric type. For the NumberOfPhysicalActivityPerWeek column we handle non-numeric entries such as "Sunday" by replacing them with numeric values (e.g., "Sunday" is replaced with 1 as we asume the person meant that they do physical activity on sunday) and then converting the column to numeric. Any remaining NA values in this column are filtered out oso that our analysis is based on valid data entries.
...
# Display the data
kable(head(survey_data), caption = "Survey data set", col.widths = rep("auto", ncol(survey_data)))
...
To handle outliers, we calculate the interquartile range (IQR) and filter out values outside the lower and upper bounds. For the transformed dataset, extreme values are scaled down by dividing them by 20, ensuring a more uniform distribution.
...
survey_data_no_outliers <- survey_data %>%
  filter(NumberOfPhysicalActivityPerWeek >= lower_bound & 
           NumberOfPhysicalActivityPerWeek <= upper_bound)

kable(head(survey_data_no_outliers), 
      caption = "Data with removed outliers in physical activity", col.widths = rep("auto", ncol(survey_data_no_outliers)))
...
kable(head(survey_data_transformed), 
      caption = "Data with transformed physical activity", col.widths = rep("auto", ncol(survey_data_transformed)))
```
...
# Research Question 1: Individuals who engage in regular physical activity will demonstrate a higher level of productivity in completing tasks for university or work compared to those who do not engage in regular physical activity.
...
ggplot(survey_data_transformed, aes(x = NumberOfPhysicalActivityPerWeek, y = WeeklyHours)) +
  geom_point() +
  geom_smooth(method = "lm", formula = y ~ x, se = TRUE, color = "blue") +
  ...
  theme_minimal()
ggplot(survey_data_no_outliers, aes(x = NumberOfPhysicalActivityPerWeek, y = WeeklyHours)) +
  geom_point() +
  geom_smooth(method = "lm", formula = y ~ x, se = TRUE, color = "blue") +
  labs(title = "Scatterplot of Composite Productivity vs. Number of Physical Activities per Week",
       x = "Number of Physical Activities per Week",
...
These visualizations reinforce the conclusion that while there are trends indicating a positive relationship between physical activity and productivity, these are not statistically significant.
...
The plots shows, that we have perfect balance in the "no outlier" and a slight inbalance in "transformed" approach. The inbalance could influence the hypothesis tests.    
