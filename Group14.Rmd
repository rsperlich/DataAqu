...
summary_stats_transformed_rq2 <- survey_data_transformed %>%
  ...
  summarise(
    ...
    SD_physical = sd(NumberOfPhysicalActivityPerWeek, na.rm = TRUE),
  )
...
summary_stats_rq2 <- tibble(
  ...
  SD = c(summary_stats_no_outliers_rq2$SD_physical, 
         summary_stats_transformed_rq2$SD_physical),
  Method = c(rep("Transform", length(summary_stats_no_outliers_rq2$AgeGroup)), 
             rep("Outlier", length(summary_stats_transformed_rq2$AgeGroup)))
)
```
...
We can also visually compare both groups by a boxplot.  

```{r}
ggplot(survey_data_transformed, 
       aes(x = AgeGroup, 
           y = NumberOfPhysicalActivityPerWeek, 
           fill = AgeGroup)) +
  geom_boxplot() +
  labs(
...
ggplot(survey_data_no_outliers, 
  ...
  labs(
    title = "Activities per Week by Age Group", 
    subtitle = "with no outliers",
    x = "Age Group", 
    y = "Activities per Week") +
  theme(legend.position = "none")
```

Here, we can see, that the difference of the groups is smaller with the "no outliers" approach, but both groups again show a visual difference between the groups.  
...
The results show, that we do not have a significant correlation between the number of physical activities per week and the age group.
